...
When it comes to UAVs \cite{wang2025gsce} introduced a framework called GSCE, which combines guidelines, skill APIs,
constraints and examples in one prompt using step-by-step, 
Chain of Thought (CoT) reasoning together with few-shot examples to get executable code for a single drone.

While SayCan and GSCE focuses on a single robot, RoCo \cite{mandi2023roco} extends this paradigm to multi-robot systems through collaborative reasoning.
...
which generates executable mission plans through a three-stage pipeline, each implemented by a dedicated LLM agent. 
...
Furthermore, it demonstrates the effectiveness of few-shot prompting, 
where LLMs are guided by example demonstrations to improve the quality and consistency of their outputs.

The novelty of the AutoHMA-LLM framework \cite{yang2025autohma} lies in its hierarchical architecture, 
where cloud-based LLMs are responsible for decomposing and allocating mission instructions to agents, 
while also optimizing global scheduling. Device-level LLMs, located on dispatcher devices, translate 
allocated tasks into robot-specific instructions, while the robots themselves execute these tasks using 
classical control algorithms. The paper explores different coordination strategies and finds that a 
centralized hybrid model performs best, supported by continuous feedback between system layers. 

Although the framework formulates scheduling as an optimization problem, it does not produce explicit, 
time-parameterized schedules that can be directly inspected or executed. Furthermore, the AutoHMA-LLM 
framework is evaluated primarily in simulation environments, without validation on real robotic platforms. 
As a result, its performance under real-world conditions, including sensor noise, communication delays, 
and execution uncertainty, remains unverified.
...
As a result, task allocation and execution remain heuristic and may lead to suboptimal utilization of 
resources.

Taken together, these limitations highlight a key gap in current LLM-based multi-robot systems, namely the 
absence of explicit scheduling mechanisms that enable coordinated, efficient, and time-aware execution.