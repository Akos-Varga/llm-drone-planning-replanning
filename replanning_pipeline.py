# each drone has only idle or busy
 
# only idle drones are asked to confirm a task
 
# if an idle drone rejects, remove that drone from that subtask
 
# then replan using the expected availability / arrival time of all drones, including busy ones
...
drone_status = {drone: {"busy": False,
                        "subtask": None,
                        "available_time": 0.0} for drone in drones}
